...
const siteUrl = "https://mapcn-svelte.dev";
...
function getItemInstallCommand(item: RegistryItem) {
	return item.name === "map"
		? mapInstallCommand
		: `npx shadcn-svelte@latest add ${siteUrl}/r/${item.name}.json`;
}
...
export function createLlmIndexMarkdown() {
	...
	const blocks = typedRegistry.items.filter((item) => item.type === "registry:block");

	return `# mapcn-svelte

mapcn-svelte is a free, open-source shadcn-svelte-style registry of ready-to-use Svelte map components and blocks. It is built on MapLibre GL, styled with Tailwind CSS, and intended for projects that already use shadcn-svelte.
...
Then import from ${code("$lib/components/ui/map")}:
...
Install once with ${code(mapInstallCommand)}, then import these APIs from ${code("$lib/components/ui/map")}:

${components.map((item) => `- [${item.title}](${item.href}) - ${item.description}`).join("\n")}

## Blocks

${blocks.map((item) => `- [${item.title ?? item.name}](/llm/${item.name}) - install with ${code(getItemInstallCommand(item))}`).join("\n")}
`;
}

export function createLlmItemMarkdown(item: RegistryItem) {
	const installCommand = getItemInstallCommand(item);
...
Type: ${code(item.type)}
Registry item: ${siteUrl}/r/${item.name}.json
...
${formatList(item.devDependencies)}
