...
export function getNetworkSummary(nodes: EdgeNode[]): NetworkSummary {
	...
	const operational = nodes.filter((node) => node.status === "operational").length;
	const hasDown = nodes.some((node) => node.status === "down");
	const hasDegraded = nodes.some((node) => node.status === "degraded");

	const status: EdgeStatus = hasDown ? "down" : hasDegraded ? "degraded" : "operational";
	const label = hasDown
		? "Partial outage"
		: hasDegraded
			? "Degraded performance"
			: "All systems operational";
	const avgUptime = nodes.reduce((sum, node) => sum + node.uptime, 0) / Math.max(total, 1);
